...
namespace mini_ros {
  ...
  Subscriber ThreadHandler::_subscribe(std::string& topic, std::function<void(std::shared_ptr<Message>)> f)
  {
    size_t index;
    tcb_mtx.lock();
    try
  ...
  Publisher ThreadHandler::_advertise(std::string& topic)
  {
      auto publish_f = [this, topic](std::shared_ptr<Message> msg)
      {
        //std::shared_ptr<T> sMsg = std::static_pointer_cast<T>(msg);
        MessageQueue::MessagePair p(topic, msg);
  ...
  ServiceServer ThreadHandler::_advertiseService(std::string& srv_name,
  ...
  ServiceClient ThreadHandler::_serviceClient(std::string& srv_name,
    std::uint32_t timeout_ms)
  {
    auto call = [srv_name, timeout_ms](std::shared_ptr<Service> srv)
    {
      if (timeout_ms > 0)
      {
